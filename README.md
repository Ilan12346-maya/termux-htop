...
While this only reflects activity within the current user's scope rather than a total system-wide hardware load, it provides a functional alternative for monitoring Termux-related resource usage. For the standalone implementation and reference scripts, see the `termux/` directory.
